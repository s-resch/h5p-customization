...
const newFontSizeQuestion = '20pt';
const newFontSizeLabel = '20pt';
const wrapperHeight = '250px';
...
function modifyH5PElements(h5pModify) {
	...
	try {
		...
		if (h5pModify[h5pElements[0]]) {
			...
			h5pModify[h5pElements[0]].forEach(function (element) {
				...
				if (containsArabic(element)) {
					...
					element.style.fontSize = newFontSizeQuestion;
				}
			});
		}
		...
		if (h5pModify[h5pElements[1]]) {
			...
			h5pModify[h5pElements[1]].forEach(function (element) {
				// Check if the element contains arabic characters
				if (containsArabic(element)) {
					// Change the font size
					element.style.fontSize = newFontSizeLabel;
				}
			});
		}

		// Check if our third class is present
		if (h5pModify[h5pElements[2]]) {
			// Loop through all elements that belong to our third class
			h5pModify[h5pElements[2]].forEach(function (element) {
				// Check if the element contains arabic characters
				if (containsArabic(element)) {
					// Change the font size
